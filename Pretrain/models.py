...
class TotalEmbedding(nn.Module):
    ...
    def forward(self, waveforms, coords, unuse_list, stations_channel_class): 
        ...
        station_mask = unuse_list.unsqueeze(-1)
...
class FullModel(nn.Module):
    def __init__(self, 
        ...
        self.TotalEmbedding = TotalEmbedding(current_station=current_station, historical_station=None,
        ...
        self.Transformer = Transformer(total_station=current_station, 
...
def mixture_density_loss(y_true, 
                         y_pred, 
                         weight, 
...
def time_distributed_loss(y_true, 
                          y_pred, 
                          loss_func, 
                          weight, 
                          weighted_loss_threshold,
    ...
    def predict_generator(self, generator, **kwargs):
        ...
        with torch.no_grad():
            for x, y in dataloader: 
                inputs_waveforms, inputs_coords, unuse_list, _ = \
                    x[0].to(self.device), x[1].to(self.device), x[2].to(self.device).long(), y[0].to(self.device)
                outputs = self.models(inputs_waveforms, inputs_coords, unuse_list, None)  
        ...
        return self.merge_preds(preds), unuse_list.cpu().numpy()
