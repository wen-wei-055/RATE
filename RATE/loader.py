...
def station_unuse_table(
    first_station_appearance, 
    last_station_appearance, 
    event_time,
    stations_table
):
    unuse_table = np.ones(len(stations_table))
...
def load_events(
    ...
    with h5py.File(data_path, 'r') as f:
        ...
        for _, event in tqdm(event_metadata.iterrows(),total=len(event_metadata)):
            ...
            unuse_table = station_unuse_table(
                first_station_appearance, 
                last_station_appearance, 
                event_name[:14],
                stations_table
            )
            if 'unuse_table' not in data.keys(): 
                data['unuse_table'] = [unuse_table]
